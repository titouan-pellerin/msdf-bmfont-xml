 ...
 */
export function valueQueue(array) {
 ...
 */
export function roundAllValue (obj, decimal = 0, strict = false) {
 ...
 * 
 */
export function stringify(data, outputType) {
  if (outputType === "json") {
    return toJSON(data);
...
export function toBMFontXML(data) {
  let xmlData = {};
  ...
  // pages section, page shall be inserted later in module export function callback
  xmlData.pages = {};
  xmlData.pages.page = []; 
 ...
 * @returns {string} 
 */
export function stringifyArray(array, seperator = "") {
  let result = "";
 ...
 * @returns {boolean} 
 */
export function isNumeric (n, strict = false) {
...
export function insidePath (command, contours) {
 ...
 */
export function alignClockwise(contours, direction) {
  let numReversed = 0;
 ...
 * @returns {string} 
 */
export function stringifyContours(contours) {
  let shapeDesc = '';
  contours.forEach(contour => {
    shapeDesc += '{';
...
export function setTolerance(pointValue, areaValue) {
...
export function degenerate(p) {
  for (let i = 0; i < p.length - 1; i++) 
    if (same(p[i], p[i + 1])) p.splice(i, 1);
  return p.length;
} 

export function isDegenerate(contour) {
...
export function filterContours(contours) {
  let filtered = 0;
  for (let i = 0; i < contours.length; i++) {
